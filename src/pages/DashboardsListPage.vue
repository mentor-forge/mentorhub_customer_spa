<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Dashboards</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="dashboard-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/dashboards/new"
          data-automation-id="dashboard-list-new-button"
        >
        ...
        <v-card>
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="dashboard-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load dashboards: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Dashboards List Page - Showcase of mentorhub_spa_utils ease of use
 * 
...
const {
  ...
  queryKey: ['dashboards'],
  queryFn: (params) => api.getDashboards(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToDashboard(_event: unknown, { item }: { item: Dashboard }) {
  router.push(`/dashboards/${item._id}`)
}
