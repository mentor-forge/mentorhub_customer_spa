<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Notes</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12">
        <ListPageSearch
          ...
          automation-id="note-list-search"
        />
      </v-col>
    </v-row>
        ...
        <v-card>
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="note-list-load-more"
            >
...
/**
 * Notes List Page - Showcase of mentorhub_spa_utils simplicity
 * 
...
const {
  ...
  queryKey: ['notes'],
  queryFn: (params) => api.getNotes(params),
  getItemId: (item) => item._id,
  limit: 20,
})

function navigateToNote(_event: unknown, { item }: { item: Note }) {
