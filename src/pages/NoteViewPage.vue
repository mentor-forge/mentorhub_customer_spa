<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">View Note</h1>
      </v-col>
    </v-row>
      ...
      <v-col cols="12" md="8">
        <v-card>
          <v-card-text>
            ...
            <v-textarea
              :model-value="note.description || 'N/A'"
              label="Description"
              readonly
              variant="outlined"
              rows="3"
...
const noteId = computed(() => routeLocation.params.id as string)

const { data: note, isLoading, error: queryError } = useQuery({
  queryKey: ['note', noteId],
  queryFn: () => api.getNote(noteId.value),
})
