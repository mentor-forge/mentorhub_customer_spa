<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Ratings</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12">
        <ListPageSearch
          ...
          automation-id="rating-list-search"
        />
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(ratings ?? []) as unknown as Rating[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToRating"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="rating-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load ratings: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Ratings List Page - Showcase of mentorhub_spa_utils simplicity
 * 
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<Rating>({
  queryKey: ['ratings'],
  queryFn: (params) => api.getRatings(params),
  getItemId: (item) => item._id,
  limit: 20,
})

function navigateToRating(_event: unknown, { item }: { item: Rating }) {
  router.push(`/ratings/${item._id}`)
}
