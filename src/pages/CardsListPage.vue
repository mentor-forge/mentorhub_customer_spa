<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Cards</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="card-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/cards/new"
          data-automation-id="card-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(cards ?? []) as unknown as Card[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToCard"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="card-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load cards: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Cards List Page - Showcase of mentorhub_spa_utils ease of use
 * 
...
const {
  ...
  queryKey: ['cards'],
  queryFn: (params) => api.getCards(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToCard(_event: unknown, { item }: { item: Card }) {
  router.push(`/cards/${item._id}`)
}
