<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Subscriptions</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="subscription-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          ...
          data-automation-id="subscription-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(subscriptions ?? []) as unknown as Subscription[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToSubscription"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="subscription-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load subscriptions: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Subscriptions List Page - Showcase of mentorhub_spa_utils ease of use
 * 
...
const {
  ...
  queryFn: (params) => api.getSubscriptions(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToSubscription(_event: unknown, { item }: { item: Subscription }) {
