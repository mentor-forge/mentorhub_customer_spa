<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Journeys</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12">
        <ListPageSearch
          ...
          automation-id="journey-list-search"
        />
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(journeys ?? []) as unknown as Journey[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToJourney"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="journey-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load journeys: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Journeys List Page - Showcase of mentorhub_spa_utils simplicity
 * 
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<Journey>({
  queryKey: ['journeys'],
  queryFn: (params) => api.getJourneys(params),
  getItemId: (item) => item._id,
  limit: 20,
})

function navigateToJourney(_event: unknown, { item }: { item: Journey }) {
  router.push(`/journeys/${item._id}`)
}
