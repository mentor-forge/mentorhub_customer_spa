<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Edit Subscription</h1>
      </v-col>
    </v-row>
        ...
        <v-card>
          <v-card-text>
            <AutoSaveField
              ...
              automation-id="subscription-edit-name-input"
            />

            <AutoSaveField
              :model-value="subscription.description || ''"
              label="Description"
              ...
              textarea
              :rows="3"
              automation-id="subscription-edit-description-input"
            />

            <AutoSaveSelect
              :model-value="subscription.status || 'active'"
              label="Status"
              :items="statusOptions"
              :on-save="(value: string) => updateField('status', value)"
              class="mt-4"
              automation-id="subscription-edit-status-select"
            />
            ...
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field
                  :model-value="formatDate(subscription.created.at_time)"
                  label="Created"
                  readonly
                  variant="outlined"
                  density="compact"
                />
                <v-text-field
                  :model-value="subscription.created.by_user"
                  label="Created By"
                  readonly
                  variant="outlined"
                  density="compact"
              ...
              <v-col cols="12" md="6">
                <v-text-field
                  :model-value="formatDate(subscription.saved.at_time)"
                  label="Last Saved"
                  readonly
                  variant="outlined"
                  density="compact"
                />
                <v-text-field
                  :model-value="subscription.saved.by_user"
                  label="Last Saved By"
                  readonly
                  variant="outlined"
                  density="compact"
            ...
            <v-card-actions class="px-0 mt-4">
              <v-btn 
                ...
                variant="text"
                data-automation-id="subscription-edit-back-button"
              >
                Back to List
              </v-btn>
            </v-card-actions>
...
/**
 * Subscription Edit Page - Showcase of spa_utils AutoSave components
 * 
...
const subscriptionId = computed(() => routeLocation.params.id as string)

const { data: subscription, isLoading, error: queryError } = useQuery({
  queryKey: ['subscription', subscriptionId],
  queryFn: () => api.getSubscription(subscriptionId.value),
})
...
const { mutateAsync: updateSubscription } = useMutation({
  mutationFn: (data: SubscriptionUpdate) => api.updateSubscription(subscriptionId.value, data),
  onSuccess: () => {
    queryClient.invalidateQueries({ queryKey: ['subscription', subscriptionId.value] })
...
async function updateField(field: keyof SubscriptionUpdate, value: string) {
  try {
    await updateSubscription({ [field]: value })
  } catch (error) {
    throw error
  }
}
