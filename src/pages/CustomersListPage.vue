    ...
    <v-row>
      <v-col cols="12">
        <ListPageSearch
          ...
          automation-id="customer-list-search"
        />
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(customers ?? []) as unknown as Customer[]"
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="customer-list-load-more"
            >
...
/**
 * Customers List Page - Showcase of mentorhub_spa_utils simplicity
 * 
...
const {
  ...
  queryKey: ['customers'],
  queryFn: (params) => api.getCustomers(params),
  getItemId: (item) => item._id,
  limit: 20,
})

function navigateToCustomer(_event: unknown, { item }: { item: Customer }) {
