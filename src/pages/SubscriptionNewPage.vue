<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">New Subscription</h1>
      </v-col>
    </v-row>
          ...
          <v-card-text>
            <v-form ref="formRef" @submit.prevent="handleSubmit">
              <v-text-field
                ...
                persistent-hint
                required
                data-automation-id="subscription-new-name-input"
              />
              ...
              <v-textarea
                ...
                rows="3"
                class="mt-4"
                data-automation-id="subscription-new-description-input"
              />
              ...
              <v-select
                ...
                :items="statusOptions"
                class="mt-4"
                data-automation-id="subscription-new-status-select"
              />
              ...
              <v-card-actions class="px-0 mt-4">
                <v-btn 
                  @click="router.back()" 
                  variant="text"
                  data-automation-id="subscription-new-cancel-button"
                >
                  Cancel
                </v-btn>
                <v-spacer />
                <v-btn
                  ...
                  :loading="isPending"
                  :disabled="isPending"
                  data-automation-id="subscription-new-submit-button"
                >
                  Create Subscription
                </v-btn>
              </v-card-actions>
...
const form = ref<SubscriptionInput>({
  name: '',
  description: '',
  status: 'active',
})
...
const { mutate: createSubscription, isPending } = useMutation<{ _id: string }, Error, SubscriptionInput>({
  mutationFn: (data: SubscriptionInput) => api.createSubscription(data),
  onSuccess: (response: { _id: string }) => {
    queryClient.invalidateQueries({ queryKey: ['subscriptions'] })
    router.push(`/subscriptions/${response._id}`)
    errorRef.value = null
  },
  onError: (error: Error) => {
