...
describe('API Client - Subscription Endpoints', () => {
  beforeEach(() => {
    mockFetch.mockClear()
    localStorage.clear()
  ...
  it('should get subscriptions with name query', async () => {
    const mockResponse = {
      items: [],
      limit: 20,
      has_more: false,
    ...
    await api.getSubscriptions({ name: 'test' })

    expect(mockFetch).toHaveBeenCalledWith(
      '/api/subscription?name=test',
      expect.any(Object)
    )
  })
  ...
  it('should get a single subscription', async () => {
    ...
    const result = await api.getSubscription('507f1f77bcf86cd799439011')
  ...
  it('should update a subscription', async () => {
    const update: SubscriptionUpdate = { name: 'updated-name' }
    ...
    const result = await api.updateSubscription('507f1f77bcf86cd799439011', update)
  ...
  it('should handle 404 errors', async () => {
    ...
    await expect(api.getSubscription('invalid-id')).rejects.toThrow(ApiError)
  })
  ...
  it('should handle 401 errors', async () => {
    ...
    await expect(api.getSubscriptions()).rejects.toThrow('Unauthorized')
  })
