...
describe('JWT claim readers', () => {
  ...
  it('should read custom: prefixed claims as fallback', async () => {
    const token = makeJwt({ 'custom:customer_id': 'cust-custom-123', 'custom:profile_id': 'prof-custom-456' })
    localStorage.setItem('access_token', token)
    const { getStoredClaim, getStoredCustomerId, getStoredProfileId } = await import('./useAuth')
  ...
  it('should handle whitespace in customer_id and profile_id', async () => {
